...
export function createDefaultParams(): TuningParams {
  return {
    ...
    // Trigger: 사랑해
    loveRingCount:    { value: 8,  min: 1,  max: 12, step: 1,    label: '❤️ 링 개수' },
    loveNebulaCount:  { value: 8,  min: 0,  max: 12, step: 1,    label: '❤️ 성운 개수' },
    loveSpiralCount:  { value: 8,  min: 0,  max: 12, step: 1,    label: '❤️ 나선 개수' },
    ...
    // Trigger: 와/대박
    wowBurstCount:    { value: 6,  min: 1,  max: 10, step: 1,    label: '🎆 폭발 수' },
    wowSparkCount:    { value: 35, min: 5,  max: 60, step: 5,    label: '🎆 불꽃 수' },

    // Trigger: 고마워
    thanksEmberCount: { value: 28, min: 3,  max: 60, step: 1,    label: '🙏 불씨 수' },
    thanksGlowAlpha:  { value: 0.05, min: 0.01, max: 0.3, step: 0.01, label: '🙏 글로우 강도' },

    // Trigger: 미안해
    sorryDropCount:   { value: 7,  min: 2,  max: 20, step: 1,    label: '💧 물방울 수' },
    sorryMistCount:   { value: 8,  min: 0,  max: 15, step: 1,    label: '💧 안개 수' },

    // Trigger: 보고싶어
    missyouStarCount: { value: 75, min: 10, max: 150, step: 5,   label: '💜 별빛 수' },
    missyouSpiralCount:{ value: 5, min: 0,  max: 8,  step: 1,    label: '💜 나선 수' },
  };
}
