...
export class GenerativeEngine {
  ...
  private idleFrame = 0;
  private idleShowcaseCooldown = 0;
  ...
  setIdleMode(idle: boolean) {
    ...
    if (idle) {
      this.idleFrame = 0;
      this.idleShowcaseCooldown = 180; // gentle warmup
    }
  }

  /** Call every frame when in idle mode (no audio). */
  updateIdle() {
    this.time += 0.016;
    this.idleFrame++;
  ...
  /** Ambient idle: slow fade + random glowing dots + periodic nebulae + ring bursts */
  private idleAnimTick() {
    const ctx = this.accCtx;
    const W = this.canvas.width;
    const H = this.canvas.height;

    // Very slow fade to black (every 6 frames)
    ...
    // Every 90 frames: random glowing dot (subtle)
    if (this.idleFrame % 90 === 0) {
      const c = NEON_COLORS[Math.floor(Math.random() * NEON_COLORS.length)];
      const x = W * (0.1 + Math.random() * 0.8);
      const y = H * (0.1 + Math.random() * 0.8);
      const r = 0.6 + Math.random() * 1.8;
      ctx.save();
      ctx.globalAlpha = 0.012 + Math.random() * 0.015;
      ctx.fillStyle = `hsl(${c[0]}, ${c[1]}%, 65%)`;
      ctx.shadowColor = ctx.fillStyle;
      ctx.shadowBlur = r * 5;
      ctx.beginPath();
    ...
    // Every 420 frames (~7s): faded nebula
    if (this.idleFrame % 420 === 0 && this.idleFrame > 0) {
      const c = NEON_COLORS[Math.floor(Math.random() * NEON_COLORS.length)];
      this.drawFadedNebula(
    ...
    // Every 600 frames (~10s): spawn an expanding ring burst
    if (this.idleFrame % 600 === 0 && this.idleFrame > 0) {
      const c = NEON_COLORS[Math.floor(Math.random() * NEON_COLORS.length)];
      this.bursts.push({
  ...
  /** Periodically fire demo events (snap/clap/laugh/word) during idle */
  private tickIdleEventShowcase() {
    this.idleShowcaseCooldown--;
    if (this.idleShowcaseCooldown > 0) return;

    // 1.5% chance per frame after cooldown
    if (Math.random() > 0.015) return;

    // Reset cooldown (200-360 frames ≈ 3.3-6s)
    this.idleShowcaseCooldown = 200 + Math.floor(Math.random() * 160);

    const roll = Math.random();
    if (roll < 0.45) {
      // Trigger a random word event
      const words: TriggerWord[] = ['love', 'hello', 'happy', 'wow', 'thanks', 'sorry', 'missyou'];
      this.triggerSpecialEvent(words[Math.floor(Math.random() * words.length)]);
    } else if (roll < 0.63) {
      // Fake snap
      this.onSnap({ volume: 0.58, bass: 0.3, mid: 0.3, treble: 0.5, pitch: 400, isSpeaking: true, soundType: 'snap', spectralCentroid: 0.5, spectralFlatness: 0.5 } as AudioFeatures);
    } else if (roll < 0.81) {
      // Fake clap
      this.onClap({ volume: 0.62, bass: 0.5, mid: 0.4, treble: 0.3, pitch: 300, isSpeaking: true, soundType: 'clap', spectralCentroid: 0.4, spectralFlatness: 0.3 } as AudioFeatures);
    } else {
      // Fake laugh
      this.onLaugh({ volume: 0.55, bass: 0.3, mid: 0.4, treble: 0.35, pitch: 350, isSpeaking: true, soundType: 'laugh', spectralCentroid: 0.4, spectralFlatness: 0.3 } as AudioFeatures);
    }
  }
